<template>
  <div>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
      <div>
        <h4 class="mb-3 mb-md-0">Block Formatlash</h4>
      </div>
      <div class="d-flex align-items-center flex-wrap text-nowrap">
        <button
          type="button"
          class="btn btn-primary btn-icon-text mb-2 mb-md-0"
          data-toggle="modal" data-target="#createFormat"
        >
          <i
        ...
        <div class="card">
          <div class="card-body">
            ...
            <p class="card-description">
                Saytda mavjud Formatlash bo'limini o'zgartirish imkoni mavjud.
            </p>
                    ...
                    <table class="table table-bordered" v-if="!loader && formats.length > 0">
                      ...
                      <tbody>
                        <tr v-for="(format, index) in formats.filter((f) => { 
                          ...
                          <td>{{ format.title }}</td>
                          <td v-html="format.desc.substr(0, 90)"></td>
                          ...
                          <td>
                            ...
                            <button type="button" class="btn btn-danger btn-icon" @click.prevent="deleteFormat(format.id)" v-if="format.status === 'published'">
                              <i class="mdi mdi-delete"></i>
                            </button>
                            <button type="button" class="btn btn-success btn-icon" @click.prevent="deleteFormat(format.id)" v-if="format.status === 'trash'">
                              <i class="mdi mdi-check-circle"></i>
                            </button>
                          </td>
                          <UpdateFormat :data="format" :modalKey="`update${format.id}`" :languages="languages" :fetch="fetchFormat"/>
                        </tr>
                      </tbody>
                    </table>
                  </div>
    ...
    <CreateFormat modalKey="createFormat" :fetch="fetchFormat" :languages="languages" />
  </div>
</template>
...
export default {
    components: { CreateFormat, UpdateFormat },
    data: () => ({
        loader: false,
        languages: [],
        formats: []
    ...
    methods: {
        ...
        async fetchFormat () {
            this.loader = true
            await this.$store.dispatch('fetchFormats')
            this.formats = this.$store.getters.formats
            this.loader = false
        },
        async deleteFormat (id) {
          await this.$store.dispatch('deleteFormat', id)
          this.fetchFormat()
        },
        statusChange(status) {
          if(status === "published") {
