<template>
  <div>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
      <div>
        <h4 class="mb-3 mb-md-0">Dastur maqsadi</h4>
      </div>
      <div class="d-flex align-items-center flex-wrap text-nowrap">
        <button
          type="button"
          class="btn btn-primary btn-icon-text mb-2 mb-md-0"
          data-toggle="modal" data-target="#createSync"
        >
          <i
        ...
        <div class="card">
          <div class="card-body">
            ...
            <p class="card-description">
                Saytda mavjud Dastur maqsadi bo'limini o'zgartirish imkoni mavjud.
            </p>
    ...
    <Create modalKey="createSync" :fetch="fetchSyn" :languages="languages" />
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async fetchSyn () {
            this.loader = true
            await this.$store.dispatch('fetchSyn')
            this.formats = this.$store.getters.syn
            this.loader = false
        },
        async deleteSyn (id) {
          await this.$store.dispatch('deleteSyn', id)
          this.fetchSyn()
        },
        statusChange(status) {
          if(status === "published") {
